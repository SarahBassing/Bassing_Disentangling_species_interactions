  ...
  gmu <- st_read("./Data/Spatial_data/Study_GMUs.shp") 
  ...
  #'  --------------------------
  #'  Extend rivers slightly beyond current extent so GMU polygons can be split by bisecting waterways
  #'  1. Priest River line must extend beyond intersection with Pend Oreille River 
  ...
  #'  2. Kootenai River line must extend beyond US-Canada border by a smidge
  #'  Convert line to points and add row name to data frame
  mapview::mapview(list(gmu_wgs84[gmu_wgs84$NAME == 1,], kootenairiver))
  ...
  #'  3. Split GMU1 by Upper Priest River, Priest River, and Kootenai River
  gmu1_priestriver_split <- gmu_wgs84[gmu_wgs84$NAME == 1,] %>% lwgeom::st_split(longer_priestriver) %>%
  ...
  #'  4. Split GMU10A by Dwarshak Reservior
  ...
  gmu10a_dworshak_split <- gmu_wgs84[gmu_wgs84$NAME == "10A",] %>% lwgeom::st_split(dworshak) %>%
    st_collection_extract("POLYGON")
  ...
  wolf_cams <- st_read("./Outputs/Spatial_outputs/spatial_rn_wolf_locs.shp") 
  ...
  spatial_locs <- function(locs, proj) {
    locs <- arrange(locs, CamID)
    sf_locs <- st_as_sf(locs, coords = c("Long", "Lat"), crs = wgs84) %>%
      mutate(obs = seq(1:nrow(.))) %>%
      dplyr::select(c("CamID")) %>%
      st_transform(proj)
    return(sf_locs)
  }
  cams_nad83 <- lapply(cam_list, spatial_locs, proj = gmu_proj)
  
  #'  Visualize stacked wolf RN abundance estimates (June - Sept 2020 - 2022)
  ggplot() +
  ...
  kD_plot <- function(locs, n) {
    #'  Remove duplicated cameras so each location represented once (avoids lots of 0 meter distances)
  ...
  ward_like_cluster <- function(dat, k, a, nndist, a2) {
    ...
    #'  Grab camera names
    CamID_label <- as.vector(cams$CamID)
    #'  Nearest neighbors for an example camera
    print(CamID_label[nearst_cams[[10]]]) 
    ...
    colnames(adjacency_matrix) <- rownames(adjacency_matrix) <- CamID_label
  ...
  #'  k is based on 2015 total documented and suspected packs per GMU at peak of
  ...
  clusters_gmu1 <- ward_like_cluster(wolf_cams_gmu1, k = 9, a = 0.6, nndist = 10, a2 = 0.3) # weighted to favor geographic distance a bit more, RAI and geographic distance that does not account for adjacency
  mapview::mapview(clusters_gmu1, zcol = "Clusters")
  clusters_gmu6 <- ward_like_cluster(wolf_cams_gmu6, k = 4, a = 0.3, nndist = 10, a2 = 0.1) # weighted to favor geographic distance a bit more, RAI and geographic distance that does not account for adjacency
  mapview::mapview(clusters_gmu6, zcol = "Clusters")
  clusters_gmu10a <- ward_like_cluster(wolf_cams_gmu10a, k = 7, a = 0.5, nndist = 10, a2 = 0.1) # weighted to favor geographic distance a bit more, RAI and geographic distance that does not account for adjacency
  mapview::mapview(clusters_gmu10a, zcol = "Clusters")
  
  #'  Split camera data based on large waterbodies that act as barriers
  wolf_cams_gmu1_E <- st_intersection(wolf_cams_gmu1, gmu1_E) %>% dplyr::select(c("CamID", "CellID", "GMU", "Setup", "Species", "RN_n", "RN_sd", "RN_n_rn"))
  wolf_cams_gmu1_NE <- st_intersection(wolf_cams_gmu1, gmu1_NE) %>% dplyr::select(c("CamID", "CellID", "GMU", "Setup", "Species", "RN_n", "RN_sd", "RN_n_rn"))
  wolf_cams_gmu1_C <- st_intersection(wolf_cams_gmu1, gmu1_C) %>% dplyr::select(c("CamID", "CellID", "GMU", "Setup", "Species", "RN_n", "RN_sd", "RN_n_rn"))
  wolf_cams_gmu1_W <- st_intersection(wolf_cams_gmu1, gmu1_W) %>% dplyr::select(c("CamID", "CellID", "GMU", "Setup", "Species", "RN_n", "RN_sd", "RN_n_rn"))
  wolf_cams_gmu10a_N <- st_intersection(wolf_cams_gmu10a, gmu10a_N) %>% dplyr::select(c("CamID", "CellID", "GMU", "Setup", "Species", "RN_n", "RN_sd", "RN_n_rn"))
  wolf_cams_gmu10a_S <- st_intersection(wolf_cams_gmu10a, gmu10a_S) %>% dplyr::select(c("CamID", "CellID", "GMU", "Setup", "Species", "RN_n", "RN_sd", "RN_n_rn")) %>%
    #'  Add "GMU10A_P_59" back in (falls slightly outside of GMU10a boundaries)
    bind_rows(wolf_cams_gmu10a[wolf_cams_gmu10a$CamID == "GMU10A_P_59",])
  ...
  clusters_gmu1E <- ward_like_cluster(wolf_cams_gmu1_E, k = 8, a = 0.8, nndist = 10, a2 = 0.4) 
  mapview::mapview(clusters_gmu1E, zcol = "Clusters")
  clusters_gmu1NE <- ward_like_cluster(wolf_cams_gmu1_NE, k = 2, a = 0.9, nndist = 10, a2 = 0.5) 
  mapview::mapview(clusters_gmu1NE, zcol = "Clusters")
  clusters_gmu1C <- ward_like_cluster(wolf_cams_gmu1_C, k = 7, a = 0.8, nndist = 10, a2 = 0.6) 
  ...
  clusters_gmu10aN <- ward_like_cluster(wolf_cams_gmu10a_N, k = 2, a = 0.6, nndist = 10, a2 = 0.4) 
  mapview::mapview(clusters_gmu10aN, zcol = "Clusters")
  clusters_gmu10aS <- ward_like_cluster(wolf_cams_gmu10a_S, k = 7, a = 0.5, nndist = 10, a2 = 0.2) 
  ...
  starter_UDs <- function(dat) {
    ...
    #'  kern defines how to calculate the UD -- using epa to keep the UD tighter 
    #'  (bivnorm creates huge polygons for some of these)
  ...
  UDs_gmu1E_clip <- st_intersection(UDs_gmu1E, gmu_wgs84[gmu_wgs84$NAME == 1,])
  mapview::mapview(list(clusters_gmu1E, UDs_gmu1E_clip), zcol = "Clusters")
  UDs_gmu1E <- UDs_gmu1E_clip
  UDs_gmu1NE_clip <- st_intersection(UDs_gmu1NE, gmu_wgs84[gmu_wgs84$NAME == 1,])
  mapview::mapview(list(clusters_gmu1NE, UDs_gmu1NE_clip), zcol = "Clusters")
  UDs_gmu1NE <- UDs_gmu1NE_clip
  UDs_gmu1C_clip <- st_intersection(UDs_gmu1C, gmu_wgs84[gmu_wgs84$NAME == 1,])
  mapview::mapview(list(clusters_gmu1C, UDs_gmu1C_clip), zcol = "Clusters")
  UDs_gmu1C <- UDs_gmu1C_clip
  UDs_gmu1W_clip <- st_intersection(UDs_gmu1W, gmu_wgs84[gmu_wgs84$NAME == 1,])
  ...
  #'  ---------------------------
  #'  Remove portions of clusters that extend beyond GMU boundary
  UDs_gmu6_clip <- st_intersection(UDs_gmu6, gmu_wgs84[gmu_wgs84$NAME == 6,])
  ...
  #'  Remove portions of clusters that extend beyond GMU boundary
  UDs_gmu10aN_clip <- st_intersection(UDs_gmu10aN, gmu_wgs84[gmu_wgs84$NAME == "10A",])
  mapview::mapview(list(clusters_gmu10aN, UDs_gmu10aN_clip), zcol = "Clusters")
  UDs_gmu10aN <- UDs_gmu10aN_clip
  UDs_gmu10aS_clip <- st_intersection(UDs_gmu10aS, gmu_wgs84[gmu_wgs84$NAME == "10A",])
  ...
  #'  Save
  write_csv(wolf_density_tbl, "./Outputs/Cluster_wolf_RAI_density.csv")
  
  #'  Back to allowing spherical geometry
  sf::sf_use_s2(TRUE)
  
  #'  Save cluster data as shapefiles
  st_write(cam_clusters_gmu1, "./Outputs/Spatial_outputs/cam_clusters_gmu1.shp")
  st_write(cam_clusters_gmu6, "./Outputs/Spatial_outputs/cam_clusters_gmu6.shp")
  st_write(cam_clusters_gmu10a, "./Outputs/Spatial_outputs/cam_clusters_gmu10a.shp")
  st_write(gmu1_poly, "./Outputs/Spatial_outputs/cluster_polygons_gmu1.shp")
  st_write(gmu6_poly, "./Outputs/Spatial_outputs/cluster_polygons_gmu6.shp")
  st_write(gmu10a_poly, "./Outputs/Spatial_outputs/cluster_polygons_gmu10a.shp")
  ...
  cam_clusters <- bind_rows(cam_clusters_gmu1, cam_clusters_gmu6, cam_clusters_gmu10a) %>%
    ...
    distinct(CamID, GMU, Clusters) %>%
    group_by(GMU, Clusters) %>%
    summarize(ncams = n()) %>%
    ungroup()
  ...
  gmu1_clusters_og <- ggplot() +
    geom_sf(data = gmu_wgs84[gmu_wgs84$NAME == 1,], fill = NA) +
  ...
  gmu6_clusters_og <- ggplot() +
    geom_sf(data = gmu_wgs84[gmu_wgs84$NAME == 6,], fill = NA) +
  ...
  gmu10a_clusters_og <- ggplot() +
    geom_sf(data = gmu_wgs84[gmu_wgs84$NAME == "10A",], fill = NA) +
  ...
  ggsave("./Outputs/GMU1_wolf_density.tiff", gmu1_wolf_density,
         units = "in", width = 5, height = 6, dpi = 300, device = "tiff", compression = "lzw")
  ggsave("./Outputs/GMU6_wolf_density.tiff", gmu6_wolf_density,
         units = "in", width = 5, height = 6, dpi = 300, device = "tiff", compression = "lzw")
  ggsave("./Outputs/GMU10A_wolf_density.tiff", gmu10a_wolf_density,
         units = "in", width = 5, height = 6, dpi = 300, device = "tiff", compression = "lzw")
  
  #'  Next step:
  #'  Generate relative density indices for structural equation models (script 3_Relative_density_indices.R)
